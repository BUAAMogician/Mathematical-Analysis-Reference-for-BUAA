...
- 幂指函数极限（$\lim{u(x)^{v(x)}}\quad(u(x)>0)$）：$\qquad\lim{u(x)^{v(x)}}=\mathrm{e}^{\lim{v\ln{u}}} \qquad(0^0,1^{\infty},{\infty}^0\Rightarrow0\cdot\infty)\\ \lim{u(x)^{v(x)}}=\mathrm{e}^{\lim{(u-1)v}} \qquad(1^{\infty}\Rightarrow0\cdot\infty)$
